...
### 3.1 进入方式：仅靠 tool call

不加 `/plan` 斜杠命令，`enter_plan_mode` / `exit_plan_mode` 是进出的唯一方式。这样更简洁，也能展示"模型自主决策何时规划"的核心概念。

### 3.2 工具限制：软约束

工具列表不变（模型始终看到全部 8 个工具），但 `execute_tool` 在 plan mode 下拦截写操作并返回拒绝消息。这与 Claude Code 的设计一致 — 不隐藏工具，靠权限层控制。

### 3.3 Plan 文件：结构化 Markdown + todo list

- Plan 文件固定路径 `.miniclaw/plan.md`，plan mode 下写入豁免
- 要求包含 Context / Steps / Verification 三个部分
- Steps 部分必须使用 `- [ ]` / `- [x]` 格式的 todo list
- 退出 plan mode 后，提示模型在执行过程中逐步更新 todo 状态

这样 plan.md 同时充当**规划文档**和**轻量任务跟踪器**，不需要单独的 Task 工具。
...
| 用户审批流程 | ExitPlanMode 不弹框确认，降低复杂度 |
| `prePlanMode` 模式快照 | 只有 agent/plan 两个模式，无需复杂恢复 |
| 独立的 Task 工具 | plan.md 的 todo list 已足够，且 Claude Code 中 task 与 plan 也是正交的 |
| 修改 system prompt | 指令通过 tool result 注入，更简洁且更容易理解原理 |
...
用户输入
  ...
  ▼
_repl_loop (cli.py)
  │  创建 context = {"mode": "agent", "plan_file": "...", "workspace_root": "..."}
  │
  ▼
...
- 返回 tool result 包含：规则说明、工作流程、plan.md 格式示例
...
`execute_tool` 中在分发到具体 handler 之前，先过 `_check_plan_mode` 检查：
...
3. `_is_plan_file_write(name, args, context)` → 通过（plan 文件豁免）
4. 其他 → 返回拒绝消息

### 4.5 Plan 文件豁免

`_is_plan_file_write` 对比写入目标路径与 `context["plan_file"]` 的绝对路径是否一致：

```python
def _is_plan_file_write(name, args, context):
    # 仅对 write/edit 检查
    # resolve_path 后 normpath 比较
```

plan.md 在 agent mode 下也正常可写，因为模型在执行阶段需要更新 todo list。
...
执行过程中，模型逐步将 `- [ ]` 改为 `- [x]`，plan.md 成为一个**活文档**。
...
| `_is_plan_file_write()` | `isSessionPlanFile()` | plan 文件写入豁免 |
| `_check_plan_mode()` | `checkWritePermissionForTool()` | 写操作拦截 |
| tool result 文本 | `plan_mode` / `plan_mode_exit` attachment | Prompt 注入 |
| `context["mode"]` | `toolPermissionContext.mode` | 模式状态 |
| plan.md 的 todo list | Task 系统（正交，prompt 桥接） | 任务跟踪 |

miniclaw 把 Claude Code 约 500 行 TypeScript 的 plan mode 逻辑压缩到约 100 行 Python，保留了核心设计思想：

- 模型自主进出（tool call 控制）
- 软约束（不隐藏工具，权限层拦截）
- Prompt 注入（通过 tool result 引导行为）
- Plan 文件豁免（唯一可写文件）
...
| 文件 | 职责 |
|------|------|
| `miniclaw/tools.py` | plan mode handler、写拦截逻辑、tool schema |
| `miniclaw/api.py` | context 参数透传 |
| `miniclaw/cli.py` | context 初始化、REPL 模式显示 |
| `tests/test_tools.py` | plan mode 单元测试（21 个用例） |
...
| `TestIsPlanFileWrite` | write/edit 到 plan 文件识别、其他文件不匹配、bash 不匹配 |
| `TestCheckPlanMode` | agent 模式全放行、plan 模式只读放行、plan 文件豁免、写/bash/edit 拦截 |
| `TestExecuteToolPlanMode` | 集成测试：拦截、豁免、进出 roundtrip、嵌套拒绝 |
